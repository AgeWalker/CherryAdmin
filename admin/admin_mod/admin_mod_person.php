...
if ($action=="list") {
  ...
  $Db->query="SELECT * FROM `$table_name` ORDER BY `reg_date` DESC";
  $Db->query();
  if (mysql_num_rows($Db->lQueryResult)>0) {
    ...
    //  какие поля выводить
    $fields_to_table = array('id','email','reg_date','conf_person');
    ...
    $content_mod .= '
          <form method="post" action="index.php?mod='.$mod.'&action=pereschet" name="form1">
            ...
            <tr>
              ...
              <th><h4>E-mail</h4></th>
              <th><h4>Дата регистрации</h4></th>
              <th width="10%"><h4>&nbsp;<i title="Активность" class="fa fa-check-square-o"></i>&nbsp;&nbsp;<i title="Подтверждён по e-mail" class="fa fa-envelope-o"></i>&nbsp;&nbsp;<i title="Удалить" class="fa fa-trash"></i></h4></th>
            </tr>
		...
		$content_mod .= '<p class="have_no_item">Клиентов не зарегистрировано.</p>';
  }
...
if ($action=="edit") {
  if (!@$_POST["submit"]) {
    ...
    if (mysql_num_rows($Db->lQueryResult)>0) { 
      ...
      $positions_arr = getEditformPositions($edit_form_fields, array('main','orders_list','hidden'));
      ...
      <div class="row">
        <form role="form" method="post" enctype="multipart/form-data">
          ...
          <div class="col-md-12">
            <div class="box box-danger">
              <div class="box-body">
                '.implode($positions_arr['orders_list']).'
              </div><!-- /.box-body -->
  ...
  else //обрабатываем форму
  {
    ...
    if ($saveStatus==true) {
      if ($preparedData['id']<1) {
        ...
        writeStat('Добавлен новый покупатель <a href="index.php?mod='.$mod.'&action='.$action.'&id='.$new_id.'" class="timeline_link_item">"'.$name.'"</a>','2');
      }
...
if ($action=="import")
	...
	//массив из имеющихся в базе карт
			$card_array = array();
			$Db->query="SELECT * FROM `mod_person_card` WHERE `name`!=''";
			$Db->query();
			if (mysql_num_rows($Db->lQueryResult)>0) 
			{
				while($lRes=mysql_fetch_assoc($Db->lQueryResult)) 
				{
					$card_array[]=$lRes["name"];
				}
			}
	...
	{
		if (!in_array($data->sheets[0]['cells'][$i+1][1],$card_array))
		{
			$query_insert.= "('".$data->sheets[0]['cells'][$i+1][1]."', '".$data->sheets[0]['cells'][$i+1][2]."'),";
		}
	}
	...
	if (!empty($query_insert)) 
		mysql_query("INSERT INTO `mod_person_card` (`name`, `number`) VALUES ".substr($query_insert, 0, -1));

	
	exit("<html><head><meta  http-equiv='Refresh' content='0; URL=index.php?mod=person&action=import'></head></html>");
	unlink($uploadfile);
} else {
      ...
      <div class="row">
        <form role="form" method="post" enctype="multipart/form-data">
          ...
          <div class="col-md-12">
            <div class="box box-danger">
              <div class="box-body">
                <p>В списке карт не должно быть лишних строк (логотипов, заголовков, телефонов и пр.).</p>
				<p>Столбцы должны быть в следующем порядке - <b>1. Наименование | 2. Код карты </b></p>
		  ...
		  <div class="col-md-12">
			  <div class="box box-danger">
				  <div class="box-body">
					<p>Список имеющихся в базе карт</p>';
				...
				if (mysql_num_rows($Db->lQueryResult)>0) 
				{
					while($lRes=mysql_fetch_assoc($Db->lQueryResult)) 
					{
						$content_mod.= $lRes["name"];
						if ($lRes["person"]>0) $content_mod.= ' <a href="index.php?mod=person&action=edit&id='.$lRes["person"].'">привязана</a>';
						$content_mod.= '<br/>';
					}
				}
					
				  $content_mod.= '</div><!-- /.box-body -->
				</div>
          </div>
